...
dt=1e13                                                                 #the timestep (smaller = more accuracy, more computing time) in seconds
distance_to_track= 5                                                    #how far to track the particle (kpc)
...
while distance_tracked < distance_to_track:
    ...
    #if mag(vel) >= c:
    #    print("Error: Initial velocity cannot exceed the speed of light. Currently, it is",mag(vel)/c,"times c. This may be because of a bad starting velocity or a too large timestep.")
    ...
    if r<1 or r>20:
        bfield=np.array([0,0,0]) #not enough data to extrapolate field, it was set to 0 near galactic center or past r=20kpc.
        ...
        #X-field component:
        if pos[2] >0 and r_p != r: 
            bhat_X=(pos[2]**2+(r-r_p)**2)**(-1/2)*np.array([pos[0]*(r-r_p)/r, pos[1]*(r-r_p)/r, pos[2]])
            if pos[2] <0:
                bhat_X=-1*bhat_X
            if abs(r_p) <  rc_X:
                bfield += b_X*(r_p/r)**2*((r-r_p)**2+pos[2]**2)**(-1/2)*bhat_X
            if abs(r_p) >= rc_X:
                bfield += b_X*(r_p/r)*((r-r_p)**2+pos[2]**2)**(-1/2)*bhat_X
    ...
    K=(gam*m*(c**2+gam**2*mag(vel)**2))**-1
    acc=np.array([K*(force[0]*(c**2+gam**2*vel[1]**2+gam**2*vel[2]**2) - gam**2*vel[0]*(force[1]*vel[1]+force[2]*vel[2])),
                  K*(force[1]*(c**2+gam**2*vel[0]**2+gam**2*vel[2]**2) - gam**2*vel[1]*(force[0]*vel[0]+force[2]*vel[2])),
                  K*(force[2]*(c**2+gam**2*vel[0]**2+gam**2*vel[1]**2) - gam**2*vel[2]*(force[0]*vel[0]+force[1]*vel[1]))])
