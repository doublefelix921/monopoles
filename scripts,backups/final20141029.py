...
dt=1.0e12                                                          #the timestep (smaller = more accuracy, more computing time) in seconds
...
def bfield(pos):
    ...
    B=np.array([0.0, 0.0, 0.0]) #0 for r>1, <20
    if mag(pos) > 1.0:
        #halo:
        if pos[2] >= 0.0:
            B = math.e**(-abs(pos[2])/z_0)*L(pos[2],h_disk,w_disk)* B_n*(1-L(r,r_n,w_h)) * np.array([-1*pos[1]/r, pos[0]/r, 0.0])
        else:
            B = math.e**(-abs(pos[2])/z_0)*L(pos[2],h_disk,w_disk)* B_s*(1-L(r,r_s,w_h)) * np.array([-1*pos[1]/r, pos[0]/r, 0.0])
        ...
        if pos[2] != 0: 
            ...
            if abs(r_p) <  rc_X:
                B += b_X*(r_p/r)**2*bhat_X
            if abs(r_p) >= rc_X:
                B += b_X*(r_p/r)*bhat_X
                
        #disk:
        if r>= 3.0 and r < 5.0 :
            B+=b_ring*(1-L(pos[2],h_disk,w_disk))
            
        elif r>=5.0 and r<=20.0:
            if   r >= r_i(theta,7) and r < r_i(theta,0): #region 1
                B+=(b1/r)*(1.0-L(pos[2],h_disk,w_disk))*np.array([math.sin(theta+i),-math.cos(theta+i),0.0])
                
            elif r >= r_i(theta,0) and r < r_i(theta,1): #region 2
                B+=(b2/r)*(1.0-L(pos[2],h_disk,w_disk))*np.array([math.sin(theta+i),-math.cos(theta+i),0.0])
                
            elif r >= r_i(theta,1) and r < r_i(theta,2): #region 3
                B+=(b3/r)*(1.0-L(pos[2],h_disk,w_disk))*np.array([math.sin(theta+i),-math.cos(theta+i),0.0])
                
            elif r >= r_i(theta,2) and r < r_i(theta,3): #region 4
                B+=(b4/r)*(1.0-L(pos[2],h_disk,w_disk))*np.array([math.sin(theta+i),-math.cos(theta+i),0.0])
                
            elif r >= r_i(theta,3) and r < r_i(theta,4): #region 5
                B+=(b5/r)*(1.0-L(pos[2],h_disk,w_disk))*np.array([math.sin(theta+i),-math.cos(theta+i),0.0])
                
            elif r >= r_i(theta,4) and r < r_i(theta,5): #region 6
                B+=(b6/r)*(1.0-L(pos[2],h_disk,w_disk))*np.array([math.sin(theta+i),-math.cos(theta+i),0.0])
                
            elif r >= r_i(theta,5) and r < r_i(theta,6): #region 7
                B+=(b7/r)*(1.0-L(pos[2],h_disk,w_disk))*np.array([math.sin(theta+i),-math.cos(theta+i),0.0])
                
            elif r >= r_i(theta,6) and r < r_i(theta,7): #region 8
                B+=(b8/r)*(1.0-L(pos[2],h_disk,w_disk))*np.array([math.sin(theta+i),-math.cos(theta+i),0.0])
    return B
...
def dvdt(pos,vel):
    ...
    force = q_b*bfield(pos) #In kg*kpc/s^2
    
    return (gam*m*c**2)**(-0.5)*(c**2*force - np.dot(force,vel)*vel)
...
while mag(pos) < 20.0 and distance_tracked < distance_to_halt:
    ...
    k2x=vel+0.5*k1v*dt
    k2v=dvdt(pos+0.5*k1x*dt,vel+0.5*k1v*dt)
    
    k3x=vel+0.5*k2v*dt
    k3v=dvdt(pos+0.5*k2x*dt,vel+0.5*k2v*dt)
    
    k4x=vel+k3x*dt
    k4v=dvdt(pos+k3x*dt,vel+k3v*dt)
    
    pos-=(dt/6.0)*(k1x+2*k2x+2*k3x+k4x)
    vel-=(dt/6.0)*(k1v+2*k2v+2*k3v+k4v)
    time-=dt
    
    distance_tracked += (dt/6.0)*(k1x+2*k2x+2*k3x+k4x)
...
print("Theta final:",theta_f)
print("Phi final:",phi_f)
print("v final:",mag(vel))
print()
print("r final:",mag(pos))
print("dt:",dt)
print()
print()
