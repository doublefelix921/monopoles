...
class Pos0Error(Exception):
    def __init__(self, rho, stopRadius, stopRadiusSensitivity):
        self.message = "Pos0Error: Initial position was outside the sim volume!\n" + \
            "Rho was %.4g and sim radius is" % (rho, stopRadius + stopRadiusSensitivity)
    def __str__(self):
        return repr(self.message)

class SpeedError(Exception):
    def __init__(self, vel):
        self.message = "SpeedError: mag(vel) >= c !  " + \
            "speed/c = %.4g,   gamma = %.4g" % (mag(vel)/c, gamma(vel))
    def __str__(self):
        return repr(self.message)

class TimeoutError(Exception):
    def __init__(self, TimeElapsedSeconds, TimeoutSeconds):
        self.message = "TimeoutError: One run lasted %ds, which is more than the allowed time of " + \
        "settings['TimeoutSeconds']= %ds \n" % (TimeElapsedSeconds, TimeoutSeconds)
    def __str__(self):
        return repr(self.message)

class FinalPosError(Exception):
    def __init__(self, pos, stopRadius, stopRadiusSensitivity):
        self.message = "FinalPosError: Must redo run for final position accuracy.\n" + \
            "mag(posF) was %.4gkpc, which is greater than the " + \
            "threshold of %.4g + %.4g = %.4gkpc." % \
            (mag(pos), stopRadius, stopRadiusSensitivity,
                stopRadius + stopRadiusSensitivity)
    def __str__(self):
        return repr(self.message)

class RelativisticRegimeError(Exception):
    def __init__(self, vel, Threshold):
        self.message = "RelativisticRegimeError: Was using acc_classical, but mag(vel)/c is %.4g, and " + \
        "gamma is %.4g! This is beyond the threshold of %.4g c" %  (mag(vel)/c, gamma(vel), threshold)
    def __str__(self):
        return repr(self.message)
...
def run_trajectory(pos0, vel0, dt, m, q_b, settings):
    ...
    if exit_status(pos0, settings["StopRadius"]) == True:
        raise Pos0Error(mag(pos0), settings["StopRadius"], 
            settings["StopRadiusSensitivity"])
    ...
    while mag(pos) < settings["StopRadius"]:
        ...
        if mag(vel) >= c: raise SpeedError(vel)
        if iterations % 10000 == 0:
            if time.time()-tStartSecs > settings["TimeoutSeconds"]:
                raise TimeoutError(time.time() - tStartSecs, settings["TimeoutSeconds"])
            bf = gb.get_B(pos)
            disp = pos - pos0
            ...
            if settings["Relativistic"] == False and mag(vel) > 0.2*c:
                ...
                raise RelativisticRegimeError(vel, settings["RelativisticVelThreshold"])
    ...
    if mag(pos) > settings["StopRadius"] + settings["StopRadiusSensitivity"]:
        raise FinalPosError(pos, settings["StopRadius"], settings["StopRadiusSensitivity"])
