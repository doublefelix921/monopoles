...
def r_i(T,I):                                                 #halo spiral boundary function
    print "T",T,"I",I,"r_negx",r_negx[I],"e^x",math.e**((T-math.pi)*math.tan(i)),"i",i
...
for ii in range(len(x)):
    for j in range(len(y)):
        POSITION=np.array([ii*step+minx, j*step+miny, z])
        Rvalues[ii][j]=region(POSITION)
        #print(POSITION)
        #print(Bvalues[i][j])
...
#
#plt.pcolor(x, y, Rvalues, cmap='gist_rainbow', vmin=-1.0, vmax=10.0)
#plt.title('pcolor')
