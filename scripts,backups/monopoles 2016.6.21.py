...
# monopole's path through the Bfield given by "A New Model of the Galatic Magnetic Field"
# Jansson, Farrar (2012).
...
# META PARAMETERS
writeToFile = True     # determines if initial & final data is written to a file
...
bfieldtests = False # determines whether to find avg & min/max of bfield
...
# E0, enter in GeV.
E0 = (10**11)*1.6827e-49
v0mag = E0*c/np.sqrt(c**4*m**2+E0**2)
theta0 = 1.7
phi0 = 3.14
v0 = v0mag*sphToRect(1, theta0, phi0)
dt_default = 5.0e10
...
nameofscript = "monopoles"   # name of the optional output file
...
try: 
    ...
    v0 = sphToRect(v0mag, theta0, phi0)
    ...
    print("No / not enough parameters inputted. Using default parameter values.")
...
def runTrajectory(r0,v0, dt):
    ...
    if (writeToFile == True):
        ...
        a = "data/"+nameofscript+gettimestd()+".txt"
        f = open(a, 'a')
        f.write("=====STARTING CONDITIONS FOR RUN ===== \nv_0 (kpc/s):\n")
        f.write(str(v0)+"\n")
        f.write("mag(v0):\n")
        f.write(str(mag(v0))+"\n")
        f.write("mag(v0)/c\n")
        f.write(str(mag(v0)/c)+"\n")
        f.write("E0 (kg*kpc^2/s^2):\n")
        f.write(str(E0)+"\n")
        f.write("THETA_0 (rads):\n")
        f.write(str(theta0)+"\n")
        f.write("PHI_0 (rads):\n")
        f.write(str(phi0)+"\n")
        f.write("q_b\n")
    ...
    while mag(pos) < 20.0 and distance_tracked < distance_to_halt:
        ...
        if minRbool:
            rlist = np.concatenate((rlist, np.array([mag(pos)])))
        
        if mag(vel) >= c:
            if (writeToFile == True):
                f.open("SPEED ERROR "+a, "a")
                f.write("beta was "+str(mag(vel)/c))
        ...
        if iterations % 5000 == 1:
            ...
            print("pos-r0 = [%.3g, %.3g, %.3g]. |pos-r0| = %.3gkpc" % (disp[0], disp[1], disp[2],mag(disp)))
    ...
    distance_from_start = mag(pos-r0)
...
#    print("Total time running:", time.time()-tstartsecs, "seconds.")
    ...
    print()
    if minRbool:
        print("minimum R=mag(pos) was %.3g" % min(rlist))
    if trajectorybool:
