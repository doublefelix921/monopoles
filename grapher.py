...
if Settings["DiskPlotBool"]==True:
    ...
    for rr in np.linspace(5, 20, num=50):
        for ttheta in np.linspace(0, 2*np.pi, num=500):
            howmanyregions=gb.region_debug(rr, ttheta)[0]
            
            if howmanyregions==1:
                regionnumber=(gb.region_debug(rr, ttheta)[1][1])%8+1 
...
if Settings["TrajectoryBool"]==True:
    ...
    trajectory_ax.text2D(0.00, 0.00, "x0="+str(pos0)+"\nv0="+str(vel0), 
...
if Settings["VecPlotBool"]==True:
    ...
    #            print("r is:", mag([position[0],position[1],0]))
                u[I][J][K]=gb.get_B(position)[0]*100  #these are multplied by 100
                v[I][J][K]=gb.get_B(position)[1]*100  #because they otherwise aren't
                w[I][J][K]=gb.get_B(position)[2]*100  #plotted for some reason.
...
if Settings["XYCrossBool"] == True:
    ...
    for xx in np.linspace(-21, 21, num=801):
        for yy in np.linspace(-21, 21, num=801):
            xylistBmag += [mag(gb.get_B([xx, yy, zcrossvalue]))]
            xylistx += [xx]
            xylisty += [yy]
...
if Settings["XZCrossBool"]==True:
    ...
    for xx in np.linspace(xdomain[0], xdomain[1], num=401):
        for zz in np.linspace(zdomain[0], zdomain[1], num=401):
            listBmag += [mag(gb.get_B([xx, ycrossvalue, zz]))]
            xzlistx += [xx]
            xzlistz += [zz]
